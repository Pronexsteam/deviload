...
![Главное окно](img/main-ru-v2.png)
...
![Скачивание завершено](img/download-done-ru-v2.png)

**Лог** открывает вывод yt-dlp за последний запуск, **Папка** — папку сохранения, **Обновить** — обновляет сам yt-dlp до свежей сборки *nightly* (YouTube меняется каждую неделю, стабильный релиз yt-dlp отстаёт; раз в день приложение напоминает, если встроенный yt-dlp старше двух недель).

## Вход в YouTube

### Зачем

Некоторым видео нужен вошедший аккаунт: приватные видео, контент для спонсоров канала — или просто YouTube, который после множества скачиваний с одной сети отвечает «Sign in to confirm you're not a bot» (HTTP 403). Один вход решает всё это. Ролики 18+ тоже требуют входа, но с середины 2026 года YouTube может дополнительно требовать подтверждение возраста у самого аккаунта — это не обходит ни один загрузчик: подтвердите возраст на youtube.com и попробуйте снова.
...
2. Откроется небольшое окно со страницей входа Google. Войдите как в обычном браузере (пароль, двухфакторное подтверждение, passkey — всё работает, внутри настоящий движок Microsoft Edge).
   ...
   ![Окно входа](img/signin-window-v2.png)
...
![Вход выполнен](img/signed-in-v2.png)
...
![Подтверждение выхода](img/signout-dialog-v2.png)
...
![Нет WebView2](img/wv2-missing-v2.png)
...
![Настройки](img/settings-ru-v2.png)
...
Неудавшийся элемент очереди показывает короткую причину; наведите на него (или на строку статуса) — появится подсказка. Сырой вывод yt-dlp по-прежнему под кнопкой **Лог**.

| Сообщение | Что это значит / что делать |
|---|---|
| **Нет JavaScript-движка** | `deno.exe` должен лежать рядом с `yt-dlp.exe` — он нужен YouTube для решения задач плеера. Распакуйте релиз заново или запустите `setup.bat` в папке с исходниками. |
| **Нужен вход в YouTube** | YouTube требует вход в аккаунт — нажмите **Войти в YouTube**. Если вход уже выполнен: выйдите и войдите заново, сессия могла устареть. |
| **Проверка возраста** | Видео требует Google-аккаунт с подтверждённым возрастом. Войдите; если не помогло, подтвердите возраст на youtube.com один раз в обычном браузере. |
| **Приватное видео** | Доступно только тем, кому владелец открыл доступ — войдите под таким аккаунтом. |
| **Только для спонсоров** | Войдите под аккаунтом со спонсорством этого канала. |
| **Недоступно в регионе** | Гео-блокировка — нужен VPN или прокси. |
| **Видео недоступно** | Удалено или недоступно — проверьте ссылку в браузере. |
| **Лимит запросов** | YouTube ограничил эту сеть (HTTP 429) — подождите несколько минут или войдите в аккаунт. |
| **Нет ffmpeg** | `ffmpeg.exe` и `ffprobe.exe` должны лежать рядом с `Deviload.exe`. Распакуйте релиз заново или запустите `setup.bat`. |
| **Нет такого формата** | Выберите качество ниже — у этого видео нет такого формата. |
| **Ссылка не поддерживается** | yt-dlp не поддерживает этот сайт или ссылку — проверьте адрес. |
| **Ошибка сети** | Соединение оборвалось или истекло — проверьте интернет и повторите. |
| Статус «Скачано — cookies устарели» | Выйти и войти заново. |
| Внезапно не качается ни одно видео | YouTube что-то поменял; нажмите **Обновить**, чтобы получить свежий nightly yt-dlp, и повторите. |
| В строке статуса «yt-dlp старше N дней» | Просто напоминание (раз в день): нажмите **Обновить**. Само ничего не скачивается. |
